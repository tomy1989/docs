...
<Note>
Don't have an account yet? [Sign up for MegaSend](https://megasend.co) and follow our onboarding process to connect your WhatsApp Business Account.
</Note>
...
```bash cURL
curl -X GET "https://api.megasend.co/instances/me" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co/instances/me', {
  headers: {
...
response = requests.get(
    'https://api.megasend.co/instances/me',
...
curl_setopt_array($curl, [
    CURLOPT_URL => 'https://api.megasend.co/instances/me',
...
```bash
curl -X GET "https://api.megasend.co/messages/responses?wa_message_id=wamid.HBgMTIzNDU2Nzg5MDEyMzQVAgARGBI5RkY2NzAyQUIyNEE4RkQ2AA==" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```javascript
const SANDBOX_BASE_URL = 'https://sandbox-api.megasend.co';
```
...
3. **Contact support** - Email [support@megasend.co](mailto:support@megasend.co)
4. **Check system status** - Visit our status page for any known issues
