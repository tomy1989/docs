...
console.log('Unsubscribe code:', result.unsubscribe_code);
```
...
print(f"Unsubscribe code: {result['unsubscribe_code']}")
```
...
{
  "success": true,
  "unsubscribe_code": "abc123def456ghi789xyz",
  "unsubscribe_url": "https://app.megasend.co.il/api/whatsapp-cloud-api/unsubscribe/abc123def456ghi789xyz",
...
Handle unsubscribe requests programmatically via API. This endpoint does not require authentication — it is designed to be called from public-facing pages using only the code:

<CodeGroup>

```bash cURL
curl -X POST "https://api.megasend.co.il/unsubscribe/process/abc123def456ghi789xyz"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co.il/unsubscribe/process/abc123def456ghi789xyz', {
  method: 'POST'
});
...
```python Python
response = requests.post(
    'https://api.megasend.co.il/unsubscribe/process/abc123def456ghi789xyz'
)
```
...
The public unsubscribe endpoint that customers use directly in their browser. This returns an HTML page, not JSON. The URL embedded in messages is generated with the `/api/whatsapp-cloud-api/unsubscribe/{code}` prefix via the app domain:

<CodeGroup>

```bash cURL
curl -X GET "https://app.megasend.co.il/api/whatsapp-cloud-api/unsubscribe/abc123def456ghi789xyz"
```

```javascript Browser
// Direct browser navigation — this is the URL embedded in WhatsApp messages
window.location.href = 'https://app.megasend.co.il/api/whatsapp-cloud-api/unsubscribe/abc123def456ghi789xyz';

// Or fetch the HTML content
const response = await fetch('https://app.megasend.co.il/api/whatsapp-cloud-api/unsubscribe/abc123def456ghi789xyz');
...
response = requests.get(
    'https://app.megasend.co.il/api/whatsapp-cloud-api/unsubscribe/abc123def456ghi789xyz'
)
...
class UnsubscribeManager {
  ...
  // Process unsubscribe from webhook or web page (no auth required)
