...
There are two ways to attach media to a message. Pick the one that matches how often the file is reused:

```mermaid
flowchart TD
    A[You have a file] --> B{Will you send it<br/>more than once?}
    B -->|no, one-off| C[Pass a public URL<br/>image_url, document_url, ...]
    C --> D[Meta downloads it<br/>on every send]
    B -->|yes, reused| E[POST /media-library/:instance_id/upload]
    E --> F[Stored, tagged, usage tracked]
    F --> G[Reuse in messages,<br/>templates and campaigns]
    D --> H[Customer]
    G --> H
```
...
<Note>
The upload endpoint is `multipart/form-data` for the `file` field only. The optional `description` and `tags` parameters are sent as **URL query parameters**, not as form fields.
</Note>

<CodeGroup>

```bash cURL
curl -X POST "https://api.megasend.co.il/media-library/YOUR_INSTANCE_ID/upload?description=Summer+collection+hero+image&tags=summer,collection,hero" \
...
const response = await fetch(
  `https://api.megasend.co.il/media-library/YOUR_INSTANCE_ID/upload?${params}`,
  {
    method: 'POST',
    headers: {
...
async function uploadProductImages(productId, images) {
  ...
  for (const image of images) {
    ...
    const response = await fetch(
      `https://api.megasend.co.il/media-library/${instanceId}/upload?${params}`,
      {
        method: 'POST',
  ...
  async uploadCampaignMedia(campaignName, files) {
    ...
    for (const file of files) {
      ...
      const params = new URLSearchParams({
        ...
        tags: `campaign,${campaignName},${new Date().getFullYear()}`
      });

      const response = await fetch(
        `${this.baseUrl}/media-library/${this.instanceId}/upload?${params}`,
        {
          method: 'POST',
...
async function renewMedia(mediaId) {
  ...
  const params = new URLSearchParams();
  if (media.description) params.set('description', media.description);
  if (media.tags?.length) params.set('tags', media.tags.join(','));
    ...
    try {
      ...
      const response = await fetch(
        `https://api.megasend.co.il/media-library/${instanceId}/upload`,
        {
          method: 'POST',
          headers: { 'X-MEGASEND-AUTH': token },
          body: formData  // description/tags can be added as URL query params if needed
        }
      );
...
async function validateAndUpload(file) {
  ...
  // File is valid, upload (description and tags are optional query params)
  const formData = new FormData();
  formData.append('file', file);
...
<Note>
Need help? Contact support at team@weblix.co.il
</Note>
