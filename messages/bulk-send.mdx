...
```bash cURL
curl -X GET "https://api.megasend.co.il/messages/bulk-jobs/bulk_job_abc123def456" \
  -H "X-MEGASEND-AUTH: YOUR_ACCESS_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co.il/messages/bulk-jobs/bulk_job_abc123def456', {
  headers: {
    'X-MEGASEND-AUTH': 'YOUR_ACCESS_TOKEN'
  }
});
...
response = requests.get(
    'https://api.megasend.co.il/messages/bulk-jobs/bulk_job_abc123def456',
    headers={'X-MEGASEND-AUTH': 'YOUR_ACCESS_TOKEN'}
)
...
```bash cURL
curl -X GET "https://api.megasend.co.il/messages/bulk-jobs?limit=20&status=completed" \
  -H "X-MEGASEND-AUTH: YOUR_ACCESS_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co.il/messages/bulk-jobs?limit=20&status=completed', {
  headers: {
    'X-MEGASEND-AUTH': 'YOUR_ACCESS_TOKEN'
  }
});
...
```bash cURL
curl -X POST "https://api.megasend.co.il/messages/bulk-jobs/bulk_job_abc123def456/cancel" \
  -H "X-MEGASEND-AUTH: YOUR_ACCESS_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co.il/messages/bulk-jobs/bulk_job_abc123def456/cancel', {
  method: 'POST',
  headers: {
    'X-MEGASEND-AUTH': 'YOUR_ACCESS_TOKEN'
  }
