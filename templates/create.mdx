...
```bash cURL
curl -X POST "https://api.megasend.co/templates/create" \
...
const response = await fetch('https://api.megasend.co/templates/create', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.megasend.co/templates/create',
...
```bash cURL
curl -X GET "https://api.megasend.co/templates?status=APPROVED&limit=50" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co/templates?status=APPROVED&limit=50', {
  headers: {
    'Authorization': 'Bearer YOUR_TOKEN'
  }
});
...
response = requests.get(
    'https://api.megasend.co/templates?status=APPROVED&limit=50',
    headers={'Authorization': 'Bearer YOUR_TOKEN'}
)
...
```bash cURL
curl -X GET "https://api.megasend.co/templates/template_abc123def456" \
  -H "Authorization: Bearer YOUR_TOKEN"
```

```javascript JavaScript
const response = await fetch('https://api.megasend.co/templates/template_abc123def456', {
  headers: {
    'Authorization': 'Bearer YOUR_TOKEN'
  }
});
...
```bash cURL
curl -X PUT "https://api.megasend.co/templates/template_abc123def456" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "components": [
...
```bash cURL
curl -X DELETE "https://api.megasend.co/templates/template_abc123def456" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
