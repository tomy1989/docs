...
2. **API Access Token Authentication** - For server-to-server integrations (recommended for client applications)
3. **X-MEGASEND-AUTH Header** - Simplified authentication using custom header

## JWT Token Authentication

JWT (JSON Web Token) authentication is used for user-based authentication. This method is ideal for web applications and mobile apps where users log in directly.
...
Include the JWT token in the `X-MEGASEND-AUTH` header:
...
You can authenticate using your access token with the custom `X-MEGASEND-AUTH` header:
