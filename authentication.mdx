...
```bash cURL
curl -X POST "https://cloud.weblix.co.il/auth/login/credentials" \
...
const response = await fetch('https://cloud.weblix.co.il/auth/login/credentials', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://cloud.weblix.co.il/auth/login/credentials',
    json={
        'username': 'your-email@example.com',
        'password': 'your-password'
    }
...
```php PHP
<?php
$response = file_get_contents('https://cloud.weblix.co.il/auth/login/credentials', false, stream_context_create([
    'http' => [
        'method' => 'POST',
...
```bash cURL
curl -X POST "https://cloud.weblix.co.il/auth/refresh" \
  -H "Content-Type: application/json" \
  -d '{
    "refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJf..."
  }'
...
```javascript JavaScript
const response = await fetch('https://cloud.weblix.co.il/auth/refresh', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
```bash cURL
curl -X POST "https://cloud.weblix.co.il/access-tokens/me" \
...
```javascript JavaScript
const response = await fetch('https://cloud.weblix.co.il/access-tokens/me', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://cloud.weblix.co.il/access-tokens/me',
    headers={
        'Authorization': 'Bearer YOUR_JWT_TOKEN',
        'Content-Type': 'application/json'
    },
...
```bash
curl -X GET "https://cloud.weblix.co.il/access-tokens/me" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X PUT "https://cloud.weblix.co.il/access-tokens/me/{token_id}" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://cloud.weblix.co.il/access-tokens/me/{token_id}/refresh" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X DELETE "https://cloud.weblix.co.il/access-tokens/me/{token_id}" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```javascript JavaScript
const response = await fetch('https://cloud.weblix.co.il/messages/send', {
  method: 'POST',
  headers: {
    'X-MEGASEND-AUTH': 'mega_token_abc123def456ghi789...',
    'Content-Type': 'application/json',
...
response = requests.post(
    'https://cloud.weblix.co.il/messages/send',
    headers={
        'X-MEGASEND-AUTH': 'mega_token_abc123def456ghi789...',
        'Content-Type': 'application/json'
    },
...
curl_setopt_array($ch, [
    CURLOPT_URL => 'https://cloud.weblix.co.il/messages/send',
...
```bash
curl -X GET "https://cloud.weblix.co.il/auth/me" \
