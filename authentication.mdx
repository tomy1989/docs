...
```bash cURL
curl -X POST "https://api.megasend.co/auth/login/credentials" \
...
const response = await fetch('https://api.megasend.co/auth/login/credentials', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.megasend.co/auth/login/credentials',
    json={
        'username': 'your-email@example.com',
        'password': 'your-password'
    }
...
```php PHP
<?php
$response = file_get_contents('https://api.megasend.co/auth/login/credentials', false, stream_context_create([
    'http' => [
        'method' => 'POST',
...
```bash cURL
curl -X POST "https://api.megasend.co/auth/refresh" \
  -H "Content-Type: application/json" \
  -d '{
    "refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJf..."
  }'
...
```javascript JavaScript
const response = await fetch('https://api.megasend.co/auth/refresh', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
...
```bash cURL
curl -X POST "https://api.megasend.co/access-tokens/me" \
...
```javascript JavaScript
const response = await fetch('https://api.megasend.co/access-tokens/me', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.megasend.co/access-tokens/me',
    headers={
        'Authorization': 'Bearer YOUR_JWT_TOKEN',
        'Content-Type': 'application/json'
    },
...
```bash
curl -X GET "https://api.megasend.co/access-tokens/me" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X PUT "https://api.megasend.co/access-tokens/me/{token_id}" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://api.megasend.co/access-tokens/me/{token_id}/refresh" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X DELETE "https://api.megasend.co/access-tokens/me/{token_id}" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash cURL
curl -X POST "https://api.megasend.co/messages/send" \
...
```javascript JavaScript
const response = await fetch('https://api.megasend.co/messages/send', {
  method: 'POST',
  headers: {
    'X-MEGASEND-AUTH': 'mega_token_abc123def456ghi789...',
    'Content-Type': 'application/json',
...
response = requests.post(
    'https://api.megasend.co/messages/send',
    headers={
        'X-MEGASEND-AUTH': 'mega_token_abc123def456ghi789...',
        'Content-Type': 'application/json'
    },
...
curl_setopt_array($ch, [
    CURLOPT_URL => 'https://api.megasend.co/messages/send',
...
```bash
curl -X GET "https://api.megasend.co/auth/me" \
